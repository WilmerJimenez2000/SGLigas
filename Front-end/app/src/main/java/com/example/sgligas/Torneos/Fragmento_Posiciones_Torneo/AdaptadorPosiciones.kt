...
class AdaptadorPosiciones(
    private val tablaPosiciones: List<EquipoPosiciones>,
    private val itemClickListener: OnItemClickListener
) : RecyclerView.Adapter<AdaptadorPosiciones.ViewHolder>() {
    interface OnItemClickListener {
        fun onItemClick(equipo: EquipoPosiciones)
    }

    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val nombreEquipo: TextView =
            itemView.findViewById(R.id.textView_nombre_equipo_clasificacion)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_equipo_clasificacion, parent, false)
