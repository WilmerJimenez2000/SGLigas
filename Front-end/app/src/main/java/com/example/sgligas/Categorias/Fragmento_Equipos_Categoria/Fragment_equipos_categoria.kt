    ...
    override fun onCreateView(
        ...
        if (json.has("datos")) {
            ...
            for (element in datosArray) {
                ...
                val equipo = Equipo(idEquipo,nombreEquipo,fecha_fundacion,presidente,escudo,colores )
        ...
        val adapter = Adaptador_equipos(listaDeEquipos, object : Adaptador_equipos.OnItemClickListener {
            override fun onItemClick(equipo: Equipo) {

               // Toast.makeText(requireContext(), "${equipo.id}", Toast.LENGTH_SHORT).show();
                // Llamar a la función irActividad con la clase adecuada
                ///irActividad(InformacionEquipo::class.java, equipo.id) // Reemplaza "ActividadDestino" con la clase de destino
            }
        })
